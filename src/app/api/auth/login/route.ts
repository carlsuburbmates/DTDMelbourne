...
import { loginRequestSchema, validateRequestBody } from '@/lib/validation';
import {
  formatErrorResponse,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = await loginUser(validatedBody.email, validatedBody.password);
    ...
    return NextResponse.json(
      ...
      {
        status:
          apiError instanceof InvalidCredentialsError || apiError instanceof BadRequestError
            ? 401
            : 500,
